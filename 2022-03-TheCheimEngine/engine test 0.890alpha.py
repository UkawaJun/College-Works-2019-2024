...
def stick(pen,loc0 = [0,0],loc1= [0,0],col = [1.0,1.0,1.0],psize = 4):
    ...
    pen.goto(loc0[0],loc0[1])
    pen.penup()
def pdCatom(pen,tarcols = [100,100,100],r= 10,xy = [0,0],shade = 0.0,res = 0.08,pcol = [1,1,1]):
    tarcol = [0.0,0.0,0.0]
    sar = (1.0 - shade)/1.0
    tarcol[0] =  (0.1 + tarcols[0]*sar/255)/1.1
    tarcol[1] =  (0.1 + tarcols[1]*sar/255)/1.1
    tarcol[2] =  (0.1 + tarcols[2]*sar/255)/1.1
    intip = xy
    t_speed = res
    t = [0.0]
    lock = [0,0]   #t的状态函数变化坐标
    ...
    for in0 in range(0,t_size - 10):
        ...
        pen.pencolor((p1tovi(pcol[0],in0,t_size,tarcol[0]),
                      p1tovi(pcol[1],in0,t_size,tarcol[1]),p1tovi(pcol[1],in0,t_size,tarcol[2])))
        pen.goto(lock[0] + intip[0],lock[1]+intip[1])
        pen.pendown()
    pen.penup()
    pcol = [0.2,0.2,0.2]
    t_size = int((5*1.45)/t_speed) + 1
    for ra in range(0,75):
        parafunc(lock,t_speed,t)
    th = t[0]
    pen.pensize(3)
    tarcol = [0.0,0.0,0.0]
...
#画笔编号  指定颜色 圆尺寸 初始坐标 阴影度 分辨度 变化性坐标初始态：默认太阳光色 1，1，1  
#pdCatom(pen,[255,40,40],20,[40,-40])
...
while(True):
    ...
    #time.sleep(0.083333 - dt)   #意思是清空的滞留时刻 加上绘制一个球的时长刚好接近一帧
    c0 = time.time()
    pen.clear()
              #draw_cri(pen,20,(0.3,0.3,0.3),(0.3,0.3,0.3),40 + int(i),-40)
    c0 = time.time()
    pdCatom(pen,[255,40,40],20,[40 + int(i),-40])
    turtle.tracer(True)
    i += 1
    c1 = time.time()
    print(c1 -c0)
