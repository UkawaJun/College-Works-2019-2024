...
class Graph:
    def __init__(self,graph,size,number,center = False,dxy = [0.0,0.0],color = [150,100,150],bov = 1):
        self.bov = bov
        self.graph = 0
        if center:
    ...
    def clear(self):
        self.pen.clear()
    def can_color(self,color,shade = 0.0,fcol = [0.0,0.0,1.0],shade2 = 0.0):
        sar = (1.0 - shade)/1.0
        self.bcol[0] =  (0.1 + color[0]*sar/255)/1.1
        self.bcol[1] =  (0.1 + color[1]*sar/255)/1.1
        self.bcol[2] =  (0.1 + color[2]*sar/255)/1.1
        sar = (1.0 - shade2)/1.0
        self.fcol[0] =  (0.1 + fcol[0]*sar/255)/1.1
        self.fcol[1] =  (0.1 + fcol[1]*sar/255)/1.1
        self.fcol[2] =  (0.1 + fcol[2]*sar/255)/1.1
    ...
    def showloc(self,num = 3,fontsize = 20):
        self.pen.pd()
        self.pen.write('x['+str( float(int((self.x + self.ax )*(10**num))/(10**num))) + 
                       ']\ny['+str( float(int((self.y + self.ay)*(10**num))/(10**num)))+ ']', 
                       font =("Courier",fontsize,"bold"))
        self.pen.up()
    def showfont(self,str0,num = 3,fontsize = 15,rx =0,pencol = [0,0,0]):
        self.pen.up()
        self.pen.pencolor(tuple(pencol))
        self.pen.goto((self.x + self.ax) + rx - fontsize * 3,(self.y + self.ay) - fontsize * 1)
        self.pen.pd()
        self.pen.write(str0, font =("Courier",fontsize,"bold"))
        self.pen.up()
        self.pen.goto((self.x + self.ax),(self.y + self.ay))
...
button = [Graph('b',40,20,False,[-500,40],bov = 4.3),]
button.append(Graph('b',40,21,False,[-550,120],bov = 1.3))
button.append(Graph('b',40,22,False,[-450,120],bov = 1.3))
button.append(Graph('b',30,22,False,[-500,220],bov = 6))
button[0].can_color((78,210,150),0.0,(100,120,150))
button[1].can_color((44,83,65),0.0,(100,120,250))
button[2].can_color((44,83,65),0.0,(100,120,250))
...
while(True):
    ...
    if not bool_t[0]:
        ...
        button[0].showfont('(t):['+ str(float(int(dxs*100)/100)) + ']',2,14,-30)
        
        num = 1
        
        for a in range(0,1):
            if unit_list[a] == 1:
                button[3].showfont(str(num),2,14,-20 + 20*a +(a-1)*15,[1.0,0.0,0.0])
            else:
                button[3].showfont(str(num),2,14,-20 + 20*a +(a-1)*15,[0.0,0.0,0.0])
            num *= 10
        a0.showloc(2,10)
        a1.showloc(2,10)
